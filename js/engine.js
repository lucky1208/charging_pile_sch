...
window.EVSE_ENGINE = (function () {
  ...
  function build(P0) {
    ...
    addSched('PJ1', '交流电能表', std.meter);
    addSched('KM1', '交流主接触器', acContactorA + 'A AC-3');
    ...
    addSched('PJ2', '直流电能表', std.meter);
    addSched('RI1', '绝缘监测装置', dc.imdSpec);
    ...
    guns.forEach((gun) => {
      ...
      addSched(gun.lockTag, '枪' + gun.index + ' 电子锁', gun.lockText);
      addSched(gun.tag, '充电枪 ' + gun.index, std.name + '：' + gun.pinText + ' · ' + gun.cableText);
    });
    addSched('A1', '充电控制单元', 'CCU · ' + std.protocol);
    addSched('A2', std.physicalLayer === 'PLC' ? 'SECC 控制器' : '计费网关', aux.backendText);
    addSched('A3', '路由器 / 通信模块', aux.networkText);
    addSched('A4', '人机交互单元', aux.hmiText);
    addSched('SB1', '急停按钮', '双断点自锁，硬线切除输出使能');
    addSched('T1', '开关电源 24V', aux.psu24Text);
    addSched('T2', '开关电源 12V', aux.psu12Text);
    addSched('M2', aux.thermalName, aux.thermalText);
    ...
    if (ess.enabled) {
      ...
      addSched('A5', 'BMS 主控 BAMS', ess.bmsText);
      addSched(ess.coupling === 'ac' ? 'M3' : 'M4', ess.coupling === 'ac' ? '储能变流器 PCS' : '储能双向 DC/DC', ess.converterText);
