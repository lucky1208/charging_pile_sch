...
window.SYM = (function () {
  ...
  const C = {
    ...
    ac: '#1d4ed8',     // 交流回路 蓝
    dc: '#dc2626',     // 充电直流回路 红
    ess: '#b45309',    // 储能直流回路 琥珀
    aux: '#0e7490',    // 辅助 24V/12V 青
    ctl: '#475569',    // 控制/采样 灰虚线
    comm: '#7c3aed',   // 通信总线 紫虚线
    pe: '#15803d',     // 保护接地 绿
    anno: '#64748b',   // 注释
    grid: '#cbd5e1'
  };
  ...
  function batteryCluster(x, y, w, h, color, title, sub) {
    ...
    return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="#fffbeb" stroke="${c}" stroke-width="1.5" rx="3"/>
      ${inner}
  ...
  /* ---------- 充电枪（按标准画外形与端子） ---------- */
  function connector(cx, cy, standardId, color) {
    const c = color || C.dc;
    if (standardId === 'gb') {
      /* 国标 GB/T 20234.3：单圆盘 9 芯 */
      let s = `<circle cx="${cx}" cy="${cy}" r="27" fill="#fff" stroke="${c}" stroke-width="1.8"/>
        <circle cx="${cx}" cy="${cy}" r="22" fill="none" stroke="${c}" stroke-width="0.8"/>
        <circle cx="${cx - 11}" cy="${cy - 9}" r="6.5" fill="#fff" stroke="${c}" stroke-width="1.4"/>
        <circle cx="${cx + 11}" cy="${cy - 9}" r="6.5" fill="#fff" stroke="${c}" stroke-width="1.4"/>
        <circle cx="${cx}" cy="${cy + 2}" r="5" fill="#fff" stroke="${c}" stroke-width="1.3"/>`;
      [[-16, 10], [-6, 15], [6, 15], [16, 10], [0, -18]].forEach((p) => {
        s += `<circle cx="${cx + p[0]}" cy="${cy + p[1]}" r="3" fill="#fff" stroke="${c}" stroke-width="1"/>`;
      });
      return s;
    }
    /* CCS Combo：上部交流盘 + 下部两个直流大端子 */
    const topR = standardId === 'us' ? 17 : 19;
    let s = `<path d="M${cx - 26},${cy - 4} a26,26 0 0 1 52,0 z" fill="#fff" stroke="${c}" stroke-width="1.6"/>
      <circle cx="${cx}" cy="${cy - 8}" r="${topR}" fill="#fff" stroke="${c}" stroke-width="1.5"/>`;
    const pins = standardId === 'us' ? [[-9, -13], [9, -13], [0, -3], [-7, -1], [7, -1]] : [[-10, -15], [10, -15], [0, -16], [-11, -4], [11, -4], [0, -3]];
    pins.forEach((p) => { s += `<circle cx="${cx + p[0]}" cy="${cy + p[1]}" r="2.8" fill="#fff" stroke="${c}" stroke-width="1"/>`; });
    s += `<circle cx="${cx - 12}" cy="${cy + 20}" r="9.5" fill="#fff" stroke="${c}" stroke-width="1.6"/>
      <circle cx="${cx + 12}" cy="${cy + 20}" r="9.5" fill="#fff" stroke="${c}" stroke-width="1.6"/>`;
    return s;
  }
  ...
  return {
    ...
    legend, schedule, documentMeta, svgOpen
  };
})();
